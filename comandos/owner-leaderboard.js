...
*TOP ${len} Coins Bank* •
Tú eres el *${usersBank.indexOf(m.sender) + 1}* de *${usersBank.length}*

${sortedBank.slice(0, len).map(({ jid, bank }, i) => `${i + 1}. ${participants.some(p => jid === p.jid) ? `(${conn.getName(jid)}) wa.me/` : '@'}${jid.split`@`[0]} *${bank} Coins Bank*`).join`\n`}

`.trim()
  m.reply(text, null, { mentions: conn.parseMention(text) })
}

handler.command = ['leaderboard2', 'lb2'] 
