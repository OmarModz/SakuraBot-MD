let handler = async (m, { conn }) => {
    ...
    conn.sendFile(m.chat, pp.male, 'out.png', `*Chico*`.trim(), m, true, { contextInfo: { 'forwardingScore': 200, 'isForwarded': true, externalAdReply:{ showAdAttribution: false, title: botname, body: `h`, mediaType: 2, sourceUrl: linkgc, thumbnail: miniurl}, mentions: [m.sender]}}, { quoted: m })
  //  await conn.sendFile(m.chat, pp.male, '', '*Chico*', m)
  conn.sendFile(m.chat, pp.female, 'out.png', `*Chica*`.trim(), m, true, { contextInfo: { 'forwardingScore': 200, 'isForwarded': true, externalAdReply:{ showAdAttribution: false, title: botname, body: `h`, mediaType: 2, sourceUrl: linkgc, thumbnail: miniurl}, mentions: [m.sender]}}, { quoted: m })
 //   await conn.sendFile(m.chat, pp.female, '', '*Chica*', m)
await conn.sendMessage(m.chat, { react: { text: '✅', key: m.key }})
}
handler.help = ['par', 'ppcouple']
handler.tags = ['img']
