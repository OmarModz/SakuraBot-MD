    ...
    //    m.gold = false
        try {
            ...
            if (settings) {
                ...
                if (!('sologp' in settings)) settings.sologp = true 
        ...
        if (settings.sologp && !m.chat.endsWith('g.us') && !/jadibot|bebot|qrbot|jadibot|getcode|serbot|bots|stop|grupos|menu|off|on|info|code|saveplugin|reg|deleteplugin|savefile|join|papel|piedra|tijera/gim.test(m.text)) return 
        ...
        for (let name in global.plugins) {
            ...
            if ((usedPrefix = (match[0] || '')[0])) {
                ...
                if (!isPrems && plugin.limit && global.db.data.users[m.sender].limit < plugin.limit * 1) {
                    conn.reply(m.chat, `Se agotaron tus *Coins* 🪙`, m)
                    continue // Limit habis
                }
                if (plugin.level > _user.level) {
                    conn.reply(m.chat, `✳️ nivel requerido ${plugin.level} para usar este comando. \nTu nivel ${_user.level}`, m)
        ...
        case 'remove':
            if (chat.welcome) {
                ...
                for (let user of participants) {
                    let pp = 'https://telegra.ph/file/59a40419dd1fe16e0dd55.jpg'
                    try {
                        pp = await this.profilePictureUrl(user, 'image')
                    } catch (e) {
                    } finally {
                    let apii = await this.getFile(pp)
                    let text = (action === 'add' ? (chat.sWelcome || this.welcome || conn.welcome || 'Welcome, @user!').replace('@subject', await this.getName(id)).replace('@desc', groupMetadata.desc?.toString() || '*No hay reglas*') :
...
global.dfail = (type, m, conn) => {
    let msg = {
        rowner: '*Este comando solo puede ser utilizado por *おYᴏsᴍᴇʀ.ᴅɢ*',
